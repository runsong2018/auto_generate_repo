...
func (d *defaultGameRepository) CreateGame(ctx context.Context, data *Game) (err error) {
	if err := d.db.WithContext(ctx).Model(&Game{}).Create(data).Error; err != nil {
		return errors.Wrap(err, "failed to create Game entity")
	}
	return nil
}

func (d *defaultGameRepository) ListGames(ctx context.Context, options map[string]interface{}, query ...GameQueryOption) (data []*Game, count int64, err error) {
	exec := d.db.WithContext(ctx).Model(&Game{})
	for _, v := range query {
		exec = v(exec)
	}
	...
	if err != nil {
		return data, count, errors.Wrap(err, "failed to list the Game entities")
	}
...
type GameRepository interface {
	...
	ListGames(ctx context.Context, options map[string]interface{}, query ...GameQueryOption) (data []*Game, count int64, err error)
